        ...
        <div class="card-body">
            <div class="card">
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('testimonials.update',$testimonial->id) }}" enctype="multipart/form-data" class="forms-sample">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            <label for="exampleInputUsername1" class="form-label">Full Name</label>
                            <input type="text" name="full_name" value="{{ $testimonial->full_name }}"  class="form-control  @error('full_name') is-invalid @enderror"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="jop_title" value="{{ $testimonial->jop_title }}" class="form-control  @error('jop_title') is-invalid @enderror"
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="comment"  class="form-control" id="" cols="30" rows="10">{{ $testimonial->comment }}</textarea>
                        </div>
                        @error('comment')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control  @error('status') is-invalid @enderror" name="status"
                                ...
                                <option @selected($testimonial->status == 0) value="0">In Acvtive</option>
                            </select>
                        </div>
                        @error('status')
                            <div class="alert alert-danger">{{ $message }}</div>
                        ...
                        <div class="col-md-11 mg-t-5 mg-md-t-0">
                            ...
                            @if ($testimonial->image)
                                <img style="border-radius:50%" width="150px" height="150px" id="output" src="{{ asset($testimonial->image) }}" />
                            @else
